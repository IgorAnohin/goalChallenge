...
def run_demo(net, image_provider, height_size, cpu, track_ids):
    ...
    #print(image_provider.get(cv2.CV_CAP_PROP_FPS))
    #print(image_provider.get(cv2.CV_CAP_PROP_FRAME_WIDTH))
    #print(image_provider.get(cv2.CV_CAP_PROP_FRAME_HEIGHT))
    stride = 8
    upsample_ratio = 4
...
def magic_with_dots(frame_points):
    frame_vectors = get_pose_vectors(frame_points)
    return compare(frame_vectors, frame_vectors)
